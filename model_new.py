...
class ConstantVelocity_XY(nn.Module):
    def __init__(self,cuda=False):
        super(ConstantVelocity_XY,self).__init__()
        self.embedding_pos=nn.Linear(64,2)
    def forward(self,input_dict,mode="train"):
        input_traj=input_dict['train_traj'].cuda()
        diff1=input_traj[:,-1,:]-input_traj[:,-2,:]
        diff2=input_traj[:,-2,:]-input_traj[:,-3,:]
        vel=(diff1 +diff2)/2
        # pred_traj=[]
        out=[]
        for i in range(30):
            out.append(input_traj[:,-1,:]+ (i+1)*vel)
...
class LSTMModel_XY(nn.Module):
    def __init__(self,cuda=False):
        super(LSTMModel_XY,self).__init__()
    ...
    def forward(self,input_dict,mode="train"):
        ...
        if mode=="validate":
            # R=torch.Tensor([helper['rotation'] for helper in input_dict['helpers']]).float().cuda()
            # t=torch.Tensor([helper['mean'] for helper in input_dict['helpers']]).float().cuda()
            R=torch.Tensor([helper['rotation'] for helper in input_dict['helpers']]).float()
            t=torch.Tensor([helper['mean'] for helper in input_dict['helpers']]).float()
            if self.use_cuda:
                R=R.cuda()
                t=t.cuda()
            pred_traj=torch.matmul(R.permute(0,2,1),pred_traj.permute(0,2,1)).permute(0,2,1)
    ...
    def forward(self,input_dict,mode="train"):
        if mode=="train" or mode=="validate":
            ...
            for index,helper_dict in enumerate(input_dict['helpers']):
                ...
                start_coords=np.array(ls.interpolate(input_dict['ref_t'][index].numpy()+input_traj[index,0,1].numpy()).coords[:])
...
class Social_Model_Refined(nn.Module):
    def __init__(self,cuda=False):
        super(Social_Model_Refined,self).__init__()
        self.agent_encoder=nn.LSTM(input_size=2,hidden_size=64,batch_first=True)
        self.neighbour_encoder=nn.LSTM(input_size=2,hidden_size=64,batch_first=True)
        self.decoder_lstm=nn.LSTMCell(input_size=1,hidden_size=128)
        self.embedding_pos=nn.Linear(128,2)
        self.rel_dis_embedding=nn.Linear(2,64)
        self.multilayer_perceptron=nn.Sequential(nn.Linear(128,512),nn.ReLU(),nn.BatchNorm1d(512),
                                                nn.Linear(512,1024),nn.ReLU(),nn.BatchNorm1d(1024))
        self.multilayer_perceptron_inv=nn.Sequential(nn.Linear(1024,1024),nn.ReLU(),nn.BatchNorm1d(1024),
                                                nn.Linear(1024,64),nn.ReLU(),nn.BatchNorm1d(64))
        # self.use_cuda=cuda
    def forward(self,input_dict,mode):
        # import pdb; pdb.set_trace()
        if mode=="train" or mode=="validate":
            ...
            self.h=torch.cat((decoder_embedding,agent_embedding),axis=1)
            self.c=torch.cat((decoder_embedding,agent_c[0]),axis=1) 
            input_dec_lstm=torch.zeros(decoder_embedding.shape[0],1).cuda()
            out=[]
            for i in range(30):
                self.h,self.c=self.decoder_lstm(input_dec_lstm,(self.h,self.c))
                out.append(self.embedding_pos(self.h))
            pred_traj=torch.stack(out,dim=1)
            if mode=="validate":
                # pdb.set_trace()
...
class Social_Model_Centerline_Refined(nn.Module):
    def __init__(self,cuda=False):
        super(Social_Model_Centerline_Refined,self).__init__()
        self.agent_encoder=nn.LSTM(input_size=2,hidden_size=64,batch_first=True)
        self.neighbour_encoder=nn.LSTM(input_size=2,hidden_size=64,batch_first=True)
        self.decoder_lstm=nn.LSTMCell(input_size=1,hidden_size=160)
        self.embedding_pos=nn.Linear(160,2)
        self.rel_dis_embedding=nn.Linear(2,64)
        self.multilayer_perceptron=nn.Sequential(nn.Linear(128,512),nn.ReLU(),nn.BatchNorm1d(512),
                                                nn.Linear(512,1024),nn.ReLU(),nn.BatchNorm1d(1024))
        self.multilayer_perceptron_inv=nn.Sequential(nn.Linear(1024,1024),nn.ReLU(),nn.BatchNorm1d(1024),
